...
import { api, type CreateBehaviorRequest } from "@shared/routes";
...
export function useBehaviors(filters?: { studentId?: string; teacherId?: string }) {
  ...
  return useQuery({
    ...
    queryFn: async () => {
      ...
      const url = new URL(api.behaviors.list.path, window.location.origin);
...
export function useBehaviorStats() {
  return useQuery({
    ...
    queryFn: async () => {
      const res = await fetch(api.behaviors.stats.path);
...
export function useCreateBehavior() {
  ...
  return useMutation({
    mutationFn: async (data: CreateBehaviorRequest) => {
      const res = await fetch(api.behaviors.create.path, {
